...
region_t* extract_requests_dns(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  for (unsigned int byte_count = 0; byte_count < buf_size; byte_count++) {
    ...
    if ((mem_count >= 12) && (*(mem+mem_count) == 0)) {
      ...
      regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
      regions[region_count - 1].start_byte = cur_start;
      // cur_end += 4 may cross the buf_size due to the random mutation
      regions[region_count - 1].end_byte = cur_end < buf_size ? cur_end : buf_size - 1;
      regions[region_count - 1].state_sequence = NULL;
      regions[region_count - 1].state_count = 0;
...
klist_t(lms) *copy_kl_messages(klist_t(lms) *kl_messages) {

  klist_t(lms) *kl_messages_new = kl_init(lms);
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    if (m->mdata == NULL) PFATAL("Unable to allocate memory region to store new message");
    memcpy(m->mdata, kl_val(it)->mdata, m->msize);

    *kl_pushp(lms, kl_messages_new) = m;
  }

  return kl_messages_new;
}
...
// Network communication functions new

int net_send_naive(int sockfd, struct timeval timeout, char *mem, unsigned int len) {
  unsigned int byte_count = 0;
  int n;
  n = send(sockfd, &mem[byte_count], len, MSG_DONTROUTE);
  if (n < 0) return -1;
  byte_count += n;
  return byte_count;
}

int net_recv_naive(int sockfd, struct timeval timeout, int poll_w, char **response_buf, unsigned int *len) {
  char temp_buf[1000];
  int n;
  n = recv(sockfd, temp_buf, sizeof(temp_buf), 0);
