...
int main(int argc, char *argv[]) {
    ...
    if (argc < 4) {
        PFATAL("Usage: ./aflnet-replay packet_file protocol port "
               "[first_resp_timeout(us) [follow-up_resp_timeout(ms)]]");
    }
    ...
    if (!strcmp(argv[2], "RTSP"))
        ...
        fprintf(stderr,
                "[AFLNet-replay] Protocol %s has not been supported yet!\n",
                argv[2]);
        exit(1);
    }
    ...
    char control[100];
    while (!feof(fp) && scanf("%s", control) != EOF) {
        if (buf) {
            ck_free(buf);
            buf = NULL;
        }
    ...
    state_sequence = (*extract_response_codes)(response_buf, response_buf_size,
                                               &state_count);
