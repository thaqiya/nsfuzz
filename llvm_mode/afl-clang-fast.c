...
static void edit_params(u32 argc, char** argv) {
  ...
  u8 fortify_set = 0, asan_set = 0, x_set = 0, maybe_linking = 1, bit_mode = 0, compiling_asm = 0;
  u8 *name;
  ...
  if (getenv("NSFUZZ_PARSE_ANNOTATION")) {
    ...
    cc_params[cc_par_cnt++] = "-Xclang";
    cc_params[cc_par_cnt++] = alloc_printf("%s/nsfuzz-annotation-pass.so", obj_path);
  ...
  cc_params[cc_par_cnt++] = alloc_printf("-include%s/llvm_mode/afl-nsfuzz-rt.h", obj_path);
