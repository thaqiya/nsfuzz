...
EXP_ST u8* state_trace_bits;          /* SHM with statue value bitmap     */
EXP_ST u8* state_range_trace_bits;          /* SHM with statue value bitmap     */

EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           virgin_tmout[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
           virgin_crash[MAP_SIZE];    /* Bits we haven't seen in crashes  */

EXP_ST u8  new_bits[MAP_SIZE];        /* New bits touched by cur queue    */

static u8  var_bytes[MAP_SIZE];       /* Bytes that appear to be variable */

static s32 shm_id;                    /* ID of the SHM region             */
static s32 shm_state_id;              /* ID of the state SHM region       */
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           messages_trim_in,          /* Messages coming into the trimmer */
           messages_trim_out,         /* Messages coming outa the trimmer */
...
struct queue_entry {
  ...
  u32 generating_state_id;            /* ID of the target state at which the new seed was generated */
...
u64 init_timeout_usecs = 100000; // default, 100ms, 0.1s
u64 read_timeout_usecs = 0;
u8 net_protocol;
u8* net_ip;
u32 net_port;
...
/*add by qss*/
u32 *state_sequence = NULL;  // state sequence to store the hash of state_trace_bits
u32 state_sequence_len = 0;
u32 init_state_hash_id;
struct init_state_t {
  u16 index;
  u8 value;
};

struct init_state_t *init_state = NULL;
u32 index_state_cnt = 0;
...
u32 not_reach_raise = 0; /* SUT not reach raise(SIGSTOP), leading to read(PIPE) timeout */
timer_t timer; /* PIPE read timer */
...
static void handle_timeout(int sig) {
  ...
  if (child_pid > 0) {
    ...
    // issue SIGCONT to avoid racing (SUT STOP while fuzzer just timeout)
    ...
    char check;
    while (1) {
      int n = recv(sockfd, &check, 1, MSG_PEEK);
      if (n > 0) {
        recv(sockfd, buff, sizeof(buff), MSG_NOSIGNAL);
      } else {
        if (n == 0 || errno == ECONNRESET) actively_close = 1;
        break;
      }
    }

    // /* This could reduce the timeout issue */
    // char check;
    // recv(sockfd, &check, 1, MSG_PEEK);
    // if (errno == ENOENT) actively_close = 1;
...
/* Handle read() timeout (wait for forkserv waitpid, SIGUSR2). */
...
static void state_init_with_gvalue() {

  memset(state_trace_bits, 255, MAP_SIZE);

  for (size_t i = 0; i < index_state_cnt; i++) {
      // _log_debug(fp_debug, "index: %u\n", init_state_index[i]);
      state_trace_bits[init_state[i].index] = init_state[i].value;
  }

}

/* Initialize the implemented state machine as a graphviz graph */
/* Modify with fsm setup */
void setup_ipsm_fsm() {
  ...
  FSM = (struct state_machine *)ck_alloc(sizeof(struct state_machine));

  FSM->khms_states = kh_init(hms);

  state_init_with_gvalue();

  init_state_hash_id = hash32(state_trace_bits, MAP_SIZE, HASH_CONST);

  char initState[STATE_STR_LEN];
  snprintf(initState, STATE_STR_LEN, "%u", init_state_hash_id);

  Agnode_t *init;
  init = agnode(ipsm, initState, TRUE);
  agset(init, "color", "blue");

  // append init state
  state_append(FSM, init_state_hash_id);

  target_state_id = init_state_hash_id;
}

/* Free memory allocated to state-machine variables */
/* Modify with fsm destroy */
void destroy_ipsm_fsm() {
  agclose(ipsm);

  kh_destroy(hs32, khs_ipsm_paths);

  struct state_info_t *state;
  kh_foreach_value(FSM->khms_states, state, {ck_free(state->seeds); ck_free(state);});
  kh_destroy(hms, FSM->khms_states);

  ck_free(FSM->state_ids);
  ck_free(FSM);
}

void destroy_state_index() {
...
void setup_pipe_timer() {
  ...
  if (ret) PFATAL("setup PIPE read timer failed");
}

/* Add by qss */
void destroy_pipe_timer() {
  int ret = timer_delete(timer);
  if (ret) PFATAL("delete PIPE read timer failed");
}
...
// 计算state_sequence中的unique state hash的数量
u32 get_unique_state_count(unsigned int *state_sequence, unsigned int state_count) {
  ...
  unsigned int discard, state_hash_id, i;
  u32 result = 0;

  for (i = 0; i < state_count; i++) {
    state_hash_id = state_sequence[i];

    if (kh_get(hs32, khs_state_ids, state_hash_id) != kh_end(khs_state_ids)) {
      continue;
    } else {
      kh_put(hs32, khs_state_ids, state_hash_id, &discard);
      result++;
    }
  }
...
void update_region_annotations(struct queue_entry* q) {
  ...
  for (u32 i = 1; i <= messages_sent; i++) {
    ...
    // }
    u32 *sub_sequence = (u32 *)ck_alloc(i * sizeof(u32));
    memcpy(sub_sequence, state_sequence, i * sizeof(u32));
    q->regions[i-1].state_sequence = sub_sequence;
    q->regions[i-1].state_count = i;
  }
}
...
// CHANGE: modify with state hash traceing
void update_fuzzs() {
  unsigned int i, discard;

  u32 state_count = state_sequence_len;
...
u32 update_scores_and_select_next_state(u8 mode) {
  ...
  u32 randV = UR(state_scores[FSM->state_num - 1]);
  u32 idx = index_search(state_scores, FSM->state_num, randV);
  result = FSM->state_ids[idx];
...
// 使用对应mode选择target state，返回state hash
unsigned int choose_target_state(u8 mode) {
  u32 result = 0;

  switch (mode) {
    case RANDOM_SELECTION: //Random state selection
      selected_state_index = UR(FSM->state_num);
      result = FSM->state_ids[selected_state_index];
      break;
    case ROUND_ROBIN: //Roud-robin state selection
      result = FSM->state_ids[selected_state_index];
      selected_state_index++;
      if (selected_state_index == FSM->state_num) selected_state_index = 0;
      break;
    case FAVOR:
      ...
      if (state_cycles < 5) {
        result = FSM->state_ids[selected_state_index];
        selected_state_index++;
        if (selected_state_index == FSM->state_num) {
...
// TODO: 阅读FAVOR选种策略逻辑
struct queue_entry *choose_seed(u32 target_state_hash_id, u8 mode) {
  khint_t k;
  struct state_info_t *state;
  struct queue_entry *result = NULL;

  k = kh_get(hms, FSM->khms_states, target_state_hash_id);
  if (k != kh_end(FSM->khms_states)) {
    state = kh_val(FSM->khms_states, k);
      ...
      case FAVOR:
        if (state->seeds_count > 10) {
          ...
          while (passed_cycles < 5) {
            ...
            //current target_state_hash_id was targeted
            if (result->generating_state_id != target_state_hash_id && !result->is_initial_seed && UR(100) < 90) continue;

            u32 target_state_index = get_state_index(FSM, target_state_hash_id);
            if (pending_favored) {
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run) {
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
    ...
    if (state_count > 0) {
      unsigned int prevStateID = init_state_hash_id;

      for(i = 0; i < state_count; i++) {
        unsigned int curStateID = state_sequence[i];
        char fromState[STATE_STR_LEN], toState[STATE_STR_LEN];
        snprintf(fromState, STATE_STR_LEN, "%u", prevStateID);
        snprintf(toState, STATE_STR_LEN, "%u", curStateID);
        ...
        Agnode_t *from, *to;
          ...
          //CHANGE: update with append interface
          state_append(FSM, prevStateID);
          ...
          // state_ids = (u32 *) ck_realloc(state_ids, (FSM->state_num + 1) * sizeof(u32));
          // state_ids[FSM->state_num++] = prevStateID;
          ...
          // //Insert this into the state_ids array too
          // state_ids = (u32 *) ck_realloc(state_ids, (FSM->state_num + 1) * sizeof(u32));
          // state_ids[FSM->state_num++] = curStateID;
  ...
  k = kh_get(hms, FSM->khms_states, init_state_hash_id);
  if (k != kh_end(FSM->khms_states)) {
    state = kh_val(FSM->khms_states, k);
  ...
  for(i = 0; i < q->region_count; i++) {
    ...
    if (regional_state_count > 0) {
      ...
      k = kh_get(hms, FSM->khms_states, reachable_state_id);
      if (k != kh_end(FSM->khms_states)) {
        state = kh_val(FSM->khms_states, k);
        state->seeds = (void **) ck_realloc (state->seeds, (state->seeds_count + 1) * sizeof(void *));
        state->seeds[state->seeds_count] = (void *)q;
        ...
        //CHANGE: use fsm append interface
        state_append(FSM, reachable_state_id);
        ...
        // k = kh_put(hms, FSM->khms_states, reachable_state_id, &discard);
        // kh_value(FSM->khms_states, k) = newState;

        // //Insert this into the state_ids array too
        // state_ids = (u32 *) ck_realloc(state_ids, (FSM->state_num + 1) * sizeof(u32));
        // state_ids[FSM->state_num++] = reachable_state_id;

        if (reachable_state_id != 0) expand_was_fuzzed_map(1, 0);
      }
      was_fuzzed_map[get_state_index(FSM, reachable_state_id)][q->index] = 0; //Mark it as reachable but not fuzzed
    }
  }
  ...
  for(i = 0; i < state_count; i++) {
    ...
    if (kh_get(hs32, khs_state_ids, state_id) != kh_end(khs_state_ids)) {
      ...
      k = kh_get(hms, FSM->khms_states, state_id);
      if (k != kh_end(FSM->khms_states)) {
        kh_val(FSM->khms_states, k)->paths++;
      }
    }
  }
  kh_destroy(hs32, khs_state_ids);
  ...
  if (!dry_run) {
    k = kh_get(hms, FSM->khms_states, target_state_id);
    if (k != kh_end(FSM->khms_states)) {
      kh_val(FSM->khms_states, k)->paths_discovered++;
    }
  }
...
/* Send (mutated) messages in order to the server under test by using SIGSTOP/SIGCONT sync*/
int send_over_network(int *status) {
  int n = 0, i = 1;
  int ignore_read = 1;
  int next_round = 0; // was_killed flag in fork_server
  u8 likely_buggy = 0;
  ...
  // Wait a bit for the multi port server initialization
  if (local_port > 0) usleep(server_wait_usecs);

  //Clear the response buffer and reset the response buffer size
  // CHANGE: 使用state sequence构建消息序列
  if (state_sequence) {
    ck_free(state_sequence);
    state_sequence = NULL;
    state_sequence_len = 0;
  }
  ...
  _log_debug(fp_debug, "[*] socket init finished\n");
  ...
  _log_debug(fp_debug, "[*] socket bind finished\n");

  if(connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {

    _log_debug(fp_debug, "connect errno: %d\n", errno);

    // If it cannot connect to the server under test
    // try it again as the server initial startup time is varied
    for (n = 0; n < 1000; n++) {
      if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) == 0) break;
      _log_debug(fp_debug, "connect times: %d, errno: %d\n", n, errno);
      // below not work (can not get in), do not know why
      // if (errno == EBADF) return 0;
      // if (errno == EALREADY) {
      //   _log_debug(fp_debug, "114\n");
      // }
      usleep(1000);
    }
    if (n == 1000) {
  ...
  _log_debug(fp_debug, "[*] socket connect finished\n");

  signal(SIGUSR2, handle_read_timeout);

  memset(&time_value, 0, sizeof(struct itimerspec));
  // if set init_timeout_usecs = 0 & init_timeout_usecs > 1000000, the timer seems not work, do not know why
  time_value.it_value.tv_sec = init_timeout_usecs / 1000000;
  time_value.it_value.tv_nsec = (init_timeout_usecs % 1000000) * 1000;
  _log_debug(fp_debug, "[*] set timer(us): %lu\n", time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);

  if (timer_settime(timer, 0, &time_value, NULL) != 0) {
      _log_debug(fp_debug, "settimer errno: %d %lu %lu %p\n", errno, time_value.it_value.tv_sec, time_value.it_value.tv_nsec, timer);
  }

  _log_debug(fp_debug, "[*] read child status from forkserver during init stage\n");

  // use signal to sync. This step should never be timeout
  if (read(fsrv_st_fd, status, 4) != 4) {
    if (stop_soon) return 0;
    PFATAL("server init stage, signal sync failed.");
  }

  _log_debug(fp_debug, "[*] status during init stage: %d\n", *status);

  if (timer_gettime(timer, &time_value) != 0) {
    _log_debug(fp_debug, "gettimer errno: %d %lu %lu %p\n", errno, time_value.it_value.tv_sec, time_value.it_value.tv_nsec, timer);
  }

  u64 read_init_us = init_timeout_usecs - (time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);

  _log_debug(fp_debug, "read time in server init stage(us): %llu, not_reach_raise : %d\n", read_init_us, not_reach_raise);
  _log_debug(fp_debug, "remain timer(us): %lu\n", time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);

  time_value.it_value.tv_sec = 0;
  time_value.it_value.tv_nsec = 0;
  timer_settime(timer, 0, &time_value, NULL);

  if (!WIFSTOPPED(*status)) {

      _log_debug(fp_debug, "[*] Not stopped during init stage, handshake failed.\n");

      return ignore_read;
  }

  if (kl_messages->size == 1) next_round = 2;
  _log_debug(fp_debug, "[*] write next_round info to forkserver during init stage\n");

  if (write(fsrv_ctl_fd, &next_round, 4) != 4) {
    if (stop_soon) return 0;
    PFATAL("server init stage, write failed to forkserver");
  }

  //write the request messages
  kliter_t(lms) *it;
  _log_debug(fp_debug, "total messages num: %d\n", kl_messages->size);
  messages_sent = 0;
  
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {

    _log_debug(fp_debug, "[*] In send loop ---------------------------------------\n");

    _log_debug(fp_debug, "[*] read child_pid from forkserver\n");
    // start a new AFL loop from reading child pid
    if (read(fsrv_st_fd, &child_pid, 4) != 4) {
      if (stop_soon) return 0;
      PFATAL("Unable to request new process from fork server in send_over_net (get pid)");
    }

    _log_debug(fp_debug, "received child_pid form forkserver\n");

    // n = net_send(sockfd, timeout, kl_val(it)->mdata, kl_val(it)->msize);
    n = send(sockfd, kl_val(it)->mdata, kl_val(it)->msize, MSG_NOSIGNAL);
    
    _log_debug(fp_debug, "net send packet %d, len: %d: %s\n", messages_sent, kl_val(it)->msize, kl_val(it)->mdata);
    ...
    if (n != kl_val(it)->msize) {
      _log_debug(fp_debug, "something wrong leading to incomplete message sent, %d, %lu, errno: %d\n", n, kl_val(it)->msize, errno);
      // PFATAL("something wrong leading to incomplete message sent");
      ignore_read = 0;
    ...
    signal(SIGUSR2, handle_timeout);

    memset(&time_value, 0, sizeof(struct itimerspec));

    time_value.it_value.tv_sec = 0;
    time_value.it_value.tv_nsec = (dry_stage ? init_timeout_usecs : read_timeout_usecs) % 1000000 * 1000;

    timer_settime(timer, 0, &time_value, NULL);

    _log_debug(fp_debug, "[*] set timer(us): %lu\n", time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);

    _log_debug(fp_debug, "[*] read child status from forkserver\n");
    s32 res;
    if ((res = read(fsrv_st_fd, status, 4)) != 4) {
      if (stop_soon) return 0;
      _log_debug(fp_debug, "read error, status: %d\n", *status);
      RPFATAL(res, "Unable to communicate with fork server in send_over_net");
    }
    _log_debug(fp_debug, "after read pipe, child status: %d, is stop? (WIFSTOPPED): %d\n", *status, WIFSTOPPED(*status));
    _log_debug(fp_debug, "WIFSIGNALED: %d\n", WIFSIGNALED(*status));
    _log_debug(fp_debug, "WTERMSIG: %d\n", WTERMSIG(*status));

    timer_gettime(timer, &time_value);

    u64 read_us = 0;
    if (dry_stage && !child_timed_out) {
      read_us = init_timeout_usecs - (time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);
      read_timeout_usecs = max(read_timeout_usecs, read_us + 3000);
      _log_debug(fp_debug, "read us: %llu, read_timeout_usecs: %llu\n", read_us, read_timeout_usecs);
    }
    else
      read_us = read_timeout_usecs - (time_value.it_value.tv_sec * 1000000 + time_value.it_value.tv_nsec / 1000);

    _log_debug(fp_debug, "read time(us): %llu\n", read_us);
    _log_debug(fp_debug, "read_timeout_usecs: %llu\n", read_timeout_usecs);
    _log_debug(fp_debug, "time_value.it_value.tv_sec: %lld, time_value.it_value.tv_nsec: %lld\n", time_value.it_value.tv_sec, time_value.it_value.tv_nsec);

    time_value.it_value.tv_sec = 0;
    time_value.it_value.tv_nsec = 0;
    timer_settime(timer, 0, &time_value, NULL);

    state_sequence = (u32 *)ck_realloc(state_sequence, messages_sent * sizeof(u32));
    // init new state
    // state_sequence[messages_sent - 1] = 0x42424242;
    _log_debug(fp_debug, "size of state_sequence: %d\n", messages_sent);
    ...
    // check status
    // status STOP: good, let go
    // status TERM: it == kl_end, return status to run_target, and no need to read again.
    // status CRASH: return status to run_target, and no need to read again.
    if (WIFSTOPPED(*status)) {
      _log_debug(fp_debug, "child stopped\n");
      if (kl_next(kl_next(it)) == kl_end(kl_messages)) {
        next_round = 2;
        _log_debug(fp_debug, "going to send the last message\n");
      }

      state_sequence[messages_sent - 1] = hash32(state_trace_bits, MAP_SIZE, HASH_CONST);
      _log_debug(fp_debug, "current state hash: %u\n", state_sequence[messages_sent - 1]);

      // for (size_t i = 0; i < MAP_SIZE; i++) {
      //   if (state_trace_bits[i])
      //     _log_debug(fp_debug, "index: %d\n", i);
      // }

      state_sequence_len++;

      _log_debug(fp_debug, "[*] write next_round info to forkserver\n");

      s32 res;
      if ((res = write(fsrv_ctl_fd, &next_round, 4)) != 4) {
        if (stop_soon) return 0;
        RPFATAL(res, "Unable to request new process from fork server in send_over_net");
      }

      if (child_pid <= 0)
        FATAL("child_pid <= 0 in send_over_net");

    } else { // may have crash
        _log_debug(fp_debug, "not stopped, child may finished or crashed\n");
        // if (next_round == 2) *status = 15; // simulate to killed by sigterm, may be useful in some case?
        goto HANDLE_RESPONSES; // process have terminated (normally exit or crash)
    }
  }

HANDLE_RESPONSES:

  _log_debug(fp_debug, "reach in HANDLE_RESPONSES\n");
  // net_recv(sockfd, timeout, poll_wait_msecs, &response_buf, &response_buf_size);

  // if (messages_sent > 0 && response_bytes != NULL) {
  //   response_bytes[messages_sent - 1] = response_buf_size;
  // }
  if (messages_sent > 0 && state_sequence != NULL && n == kl_val(it)->msize) {
    state_sequence[messages_sent - 1] = hash32(state_trace_bits, MAP_SIZE, HASH_CONST);
    state_sequence_len++;
    _log_debug(fp_debug, "final state: %u\n", state_sequence[messages_sent - 1]);
  }
  ...
  if (likely_buggy && false_negative_reduction) {
    _log_debug(fp_debug, "likely_buggy && false_negative_reduction\n");
    return 0;
  }

  // let the run_target trate it as FAULT_NONE
  // 正常情况下，该操作可以解除forkserver的waitpid状态，并返回正常结束的status
  if (terminate_child && (child_pid > 0)) kill(child_pid, SIGTERM); 

  //give the server a bit more time to gracefully terminate
  _log_debug(fp_debug, "kill(child_pid, 0): %d\n", kill(child_pid, 0));

  int count = 0;
  while(1) {
    int check = kill(child_pid, 0);
    if ((check != 0) && (errno == ESRCH)) break;
    count++;
    if (count > 1000) {
      /*
      case2: 有时在同步正常情况下，fuzzer send失败，但forkserver仍然正常收到sigstop，
      导致status为4991，SUT为STOP，无法直接被terminate，这种情况下也直接kill掉。
      后续outer read如何得到status仍为4991，对此特殊处理为超时，使得forkserver也能恢复同步
      */
      _log_debug(fp_debug, "case2: child stopped, SIGKILL it directly\n");
      kill(child_pid, SIGKILL);
      // count = 0;
      break;
    }
...
static void bind_to_free_cpu(void) {
  ...
  /* Scan all /host/proc/<pid>/status entries, checking for Cpus_allowed_list.
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  while (i--) {
    ...
    if (unlikely(*current) && unlikely(*current & *virgin)) {
      ...
      // save the new touched bits
      if (virgin_map == virgin_bits) *new = *current & *virgin;
...
static void remove_state_shm(void) {
...
static void cull_queue(void) {
  ...
  for (i = 0; i < MAP_SIZE; i++)
    if (top_rated[i] && (temp_v[i >> 3] & (1 << (i & 7)))) {
      ...
      _log_debug(fp_debug, "target_state_id: %u, get_state_index(FSM, target_state_id): %d, top_rated[i]->index: %d\n", target_state_id, get_state_index(FSM, target_state_id), top_rated[i]->index);
      _log_debug(fp_debug, "fuzzed_map_states: %d, fuzzed_map_qentries: %d\n", fuzzed_map_states, fuzzed_map_qentries);
      if ((top_rated[i]->generating_state_id == target_state_id || top_rated[i]->is_initial_seed) && (was_fuzzed_map[get_state_index(FSM, target_state_id)][top_rated[i]->index] == 0)) pending_favored++;
...
EXP_ST void setup_shm(void) {
  ...
  atexit(remove_state_shm);

  shm_state_str = alloc_printf("%d", shm_state_id);

  if (!dumb_mode) setenv(SHM_STATE_VAR, shm_state_str, 1);

  ck_free(shm_state_str);

  state_trace_bits = shmat(shm_state_id, NULL, 0);

  if (!state_trace_bits) PFATAL("shmat() failed");
...
static void load_state_index() {

    FILE *f = fopen(DEFAULT_GINDEX_PATH, "r");

    u8 buf[GINDEX_MAX_LEN];

    if (!f) {
      WARNF("Unable to open '%s'", DEFAULT_GINDEX_PATH);
      return;
  }

  while (fgets(buf, GINDEX_MAX_LEN, f)) {

      init_state = (struct init_state_t *)ck_realloc(init_state, (index_state_cnt + 1) * sizeof(struct init_state_t));

      u16 index;
      u8 value;
      sscanf(buf, "%hu,%hhu\n", &index, &value);

      init_state[index_state_cnt].index = index;
      init_state[index_state_cnt].value = value;
      _log_debug(fp_debug, "input: %sindex_state_cnt: %d, index: %hu, value: %hhu\n", buf, index_state_cnt, init_state[index_state_cnt].index, init_state[index_state_cnt].value);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  _log_debug(fp_debug, "init state hash: %u\n", hash32(state_trace_bits, MAP_SIZE, HASH_CONST));

  // for (size_t i = 0; i < MAP_SIZE; i++) {
  //     if (state_trace_bits[i]) _log_debug(fp_debug, "index: %d\n", i);
  // }
  ...
  _log_debug(fp_debug, "start a new testcase, child pid: %d\n", child_pid);
  ...
  _log_debug(fp_debug, "set outer timer\n");
  /* The SIGALRM handler simply kills the child_pid and sets child_timed_out. */
  int ignore_read = 0;
  if (dumb_mode == 1 || no_forkserver) {
    if (use_net) 
      send_over_network(&status);
    if (waitpid(child_pid, &status, 0) <= 0) PFATAL("waitpid() failed");

  } else {
    if (use_net) ignore_read = send_over_network(&status);

    _log_debug(fp_debug, "finished a testcase.\n");
    _log_debug(fp_debug, "ignore_read: %d\n", ignore_read);
    ...
    if (!ignore_read) {
      ...
      _log_debug(fp_debug, "outer read status: %d\n", status);
      // 此时的status如果为4991，则出现了case2
      // 然而实际上此时child已被fuzzer强行SIGKILL掉，覆盖status并模拟为timeout
      if (WIFSTOPPED(status)) {
        _log_debug(fp_debug, "case2 happen! simulate timeout to resync the fuzzer and forkserver\n");
        child_timed_out = 1;
        status = 9;
      }
    }
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    ...
    if (child_timed_out && kill_signal == SIGKILL) {
      ...
      if (actively_close) return FAULT_NONE;

      _log_debug(fp_debug, "timeout mark\n");
      return FAULT_TMOUT;
    }
...
static u8 trim_case(char** argv) {
  ...
  u8  fault = 0;
  u32 trim_exec = 0;
  u32 remove_message_num = 0;

  /* Although the trimmer will be less useful when variable behavior is
     detected, it will still work to some extent, so we don't check for
     this. */

  if (messages_sent < max_seed_region_count) return 0;

  u32 base_unique_state_count = get_unique_state_count(state_sequence, state_sequence_len);

  stage_name = tmp;
  messages_trim_in += messages_sent;

  kliter_t(lms) *pre = NULL;

  kliter_t(lms) *it = kl_begin(kl_messages);

  sprintf(tmp, "trim %s/%s", DI(messages_sent), DI(messages_sent));

  stage_cur = 0;
  stage_max = messages_sent;

  _log_debug(fp_debug, "start to trim testcase, message_sent before trimming: %d\n", messages_sent);

  while (it != kl_end(kl_messages) && kl_messages->size > 1) {

      u32 unique_state_count = 1;

      if (pre == NULL) {
        kl_begin(kl_messages) = kl_next(it);
      } else {
        kl_next(pre) = kl_next(it);
      }
      --kl_messages->size;
      ...
      // check whether trace_bits have cover all new_bits
      ...
      while (i--) {
        // if (*new) _log_debug(fp_debug, "current: 0x%08x, new: 0x%08x\n", *current, *new);
        if (*new && ~*current & *new) {
            bits_cover = 0;
            break;
        }
      ...
      if (state_aware_mode) {
          unique_state_count = get_unique_state_count(state_sequence, state_sequence_len);
      }
      ...
      _log_debug(fp_debug, "bits_cover: %d\n", bits_cover);

      if (bits_cover && (state_aware_mode ? unique_state_count == base_unique_state_count : 1)) {

        _log_debug(fp_debug, "message trimmed: %s\n", kl_val(it)->mdata);

        kliter_t(lms) *remove = it;
        ...
        kl_next(remove) = NULL;
        ck_free(kl_val(remove)->mdata);
        ck_free(kl_val(remove));
        kmp_free(lms, kl_messages->mp, remove);

        remove_message_num++;

      } else {

          _log_debug(fp_debug, "message %d not trimmed\n", stage_cur);

          if (pre == NULL) {
              kl_begin(kl_messages) = it;
          } else {
              kl_next(pre) = it;
          }
  ...
  _log_debug(fp_debug, "total trimmed message num: %d\n", remove_message_num);

  // run once to get final trace_bits coverage (to get a normal calibrate)
  fault = run_target(argv, exec_tmout);

abort_trimming:

  messages_trim_out += messages_sent;
  return fault;
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    // we try to trim message here!
    // copy another kl_messages
    klist_t(lms) *kl_messages_ori = kl_messages;
    klist_t(lms) *kl_messages_copy = copy_kl_messages(kl_messages);
    kl_messages = kl_messages_copy;

    res = trim_case(argv);
    if (res == FAULT_ERROR)
      FATAL("Unable to execute target application during trimming");
    ...
    //resume the kl_messages
    kl_messages = kl_messages_ori;
    delete_kl_messages(kl_messages_copy);
  }
...
static double get_runnable_processes(void) {
  ...
  /* On Linux, /host/proc/stat is probably the best way; load averages are
...
#define SP20    SP10 SP10
  ...
  if (!messages_trim_out) {
    ...
    sprintf(tmp, "%0.02f%%/%s, ",
            ((double)(messages_trim_in - messages_trim_out)) * 100 / messages_trim_in,
            DI(trim_execs));
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  // _log_debug(fp_debug, "out_buf len: %d, region count: %d, max_seed_region_count: %d\n", len, region_count, max_seed_region_count);
  ...
  for (i = 0; i < region_count; i++) {
    ...
    // _log_debug(fp_debug, "regisons[i].start: %d, regisons[i].end: %d\n", regions[i].start_byte, regions[i].end_byte);
    // _log_debug(fp_debug, "i: %d, len: %d\n", i, len);
  ...
  fault = run_target(argv, exec_tmout);
  _log_debug(fp_debug, "fault after common fuzz: %d\n", fault);
...
AFLNET_REGIONS_SELECTION:;
  ...
  if (state_aware_mode) {
    ...
    if (target_state_id == init_state_hash_id) {
      ...
      for(i = M2_start_region_ID; i < queue_cur->region_count ; i++) {
        ...
        _log_debug(fp_debug, "queue_cur->region_count: %d, i: %d, queue_cur->regions[i].state_count: %d, target_state_id: %d\n", queue_cur->region_count, i, queue_cur->regions[i].state_count, target_state_id);
        u32 regionalStateCount = queue_cur->regions[i].state_count;
        if (regionalStateCount > 0) {
          if (queue_cur->regions[i].state_sequence[regionalStateCount - 1] != target_state_id) break;
          M2_region_count++;
        } else {
          return 1;
        }
  ...
  _log_debug(fp_debug, "M2_start_region_ID: %d, M2_region_count: %d\n", M2_start_region_ID, M2_region_count);
...
abandon_entry:
  ...
  if (!stop_soon && !queue_cur->cal_failed && !queue_cur->was_fuzzed) {
    ...
    was_fuzzed_map[get_state_index(FSM, target_state_id)][queue_cur->index] = 1;
    pending_not_fuzzed--;
...
static void check_crash_handling(void) {
  ...
  if (read(fd, &fchar, 1) == 1 && fchar == '|') {
    ...
    SAYF("\n" cLRD "[-] " cRST
         ...
         "    and temporarily modify /host/proc/sys/kernel/core_pattern, like so:\n\n"

         "    echo core >/host/proc/sys/kernel/core_pattern\n");
...
EXP_ST void setup_signal_handlers(void) {
  ...
  /* SIGUSR2: read timeout notifications */
  sa.sa_handler = handle_read_timeout;
...
int main(int argc, char** argv) {
  ...
  //char** use_argv;
  fp_debug = fopen("./aflnet_debug.txt", "w+");
  struct timeval tv;
  struct timezone tz;
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:l:")) > 0)
    ...
    switch (opt) {
      ...
      case 'w': /* receive/send socket timeout determining time waited for each response */
        ...
        if (sscanf(optarg, "%llu", &init_timeout_usecs) < 1 || optarg[0] == '-') FATAL("Bad syntax used for -w");
        socket_timeout = 1;
        break;
  ...
  if (state_aware_mode) {
    ...
    while (1) {
      ...
      while(!selected_seed || selected_seed->region_count == 0) {
        ...
        khint_t k = kh_get(hms, FSM->khms_states, target_state_id);
        if (k != kh_end(FSM->khms_states)) {
          kh_val(FSM->khms_states, k)->selected_times++;
        }
    ...
    while (1) {
      ...
      _log_debug(fp_debug, "after cull_queue\n");
      ...
      _log_debug(fp_debug, "goint to fuzz_one\n");
