...
std::string getStoreId(StoreInst *SI) {
	...
	Diag << "SIPtrValue:  " << *V << "\n";
	Type *VT = V->getType();
	Diag << "SIPtrValueType:  " << *VT << "\n";

	if (VT->isSingleValueType()) {
		Diag << "SingleValueType\n";
	} else if (VT->isAggregateType()) {
		Diag << "AggregateType\n";
	}

	Value *val = SI->getValueOperand();
	Diag << "SIvalueoperand:  " << *val << "\n";
	if (llvm::ConstantInt* cint = dyn_cast<llvm::ConstantInt>(val)) 
		Diag << "val is ConstantInt" << "\n";
	else 
		Diag << "val is not ConstantInt" << "\n";
	Type *SIT = SI->getType();
	Diag << "SIType:  " << *SIT << "\n";
...
std::string getAnnotation(Value *V, Module *M) {
	...
	while (!WorkList.empty()) {
		...
		if (GlobalVariable *GV = dyn_cast<GlobalVariable>(v)) {
			string ValId = getVarId(GV);

			Type *GVTy = GV->getType();
			Diag << "GV Type: " << *GVTy <<"\n";
			PointerType *GVPtrTy = dyn_cast<PointerType>(GVTy);
			Diag << "GV PtrTy: " << *GVPtrTy <<"\n";
			CompositeType *CT = dyn_cast<CompositeType>(GVPtrTy->getElementType());
			if (CT) {
        		Diag << "CT: "<< *CT << "\n";
				ValId = "array." + ValId;
			}
            Diag << "getVarId(GV): " << ValId << "\n";
			return ValId;
        }
		...
		if (PVal) {
			Diag << "PVal: " << *PVal << "\n";
			Diag << "PValType: " << *(PVal->getType()) << "\n";

			std::string structId = getStructId(PVal, is, ie, M);
			Diag << "structId: " << structId <<"\n";
			if (!structId.empty()) {
				return structId;
			} else {
