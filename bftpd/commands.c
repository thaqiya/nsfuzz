...
void command_retr(char *filename)
{
    ...
    if ((((statbuf.st_size - offset) * ratio_send) / ratio_recv > bytes_recvd - bytes_sent) && (strcmp((char *)config_getoption("RATIO"), "none")))
    {
        ...
        control_printf(SL_FAILURE, "553 File too big. Send at least %lf bytes first.",
                       (double)(((statbuf.st_size - offset) * ratio_send) / ratio_recv) - bytes_recvd);
        if (mapped)
            free(mapped);
        return;
    }
...
void command_rnto(char *newname)
{
    ...
    {
        if (mapped)
            free(mapped); // avoid leaking memory
