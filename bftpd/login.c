...
int bftpd_login(char *password)
{
    ...
    else if ((!file_auth[0]) && (anon_ok) && !change_uid)
    {
        ...
        bftpd_log("str after replace: %s\n", str);
        bftpd_log("home_directory: %s\n", home_directory);
        /* should we chroot? */
        if (strcasecmp(config_getoption("DO_CHROOT"), "no"))
        {
            bftpd_log("do chroot.\n");
...
int checkpass_pwd(char *password)
{
    ...
    bftpd_log("crypt passwd: %s\n", crypt_value);
    bftpd_log("userinfo.passwd: %s\n", userinfo.pw_passwd);
